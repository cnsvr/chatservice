...
(function ($) {
    ...
    // Left Sidebar Menu (Vertical Menu)
    function initLeftSidebarProd() {
        var self = this;
        if ($(".side-nav-prod").length) {
            var navCollapse = $('.side-nav-prod li .collapse');
            var navToggle = $(".side-nav-prod li [data-bs-toggle='collapse']");
            navToggle.on('click', function (e) {
                return false;
            });
            ...
            navCollapse.on({
                'show.bs.collapse': function (event) {
                    var parent = $(event.target).parents('.collapse.show');
                    $('.side-nav-prod .collapse.show').not(event.target).not(parent).collapse('hide');
                }
            });

            // activate the menu in left side bar (Vertical Menu) based on url
            $(".side-nav-prod a").each(function () {
                var pageUrl = window.location.href.split(/[?#]/)[0];
                if (this.href == pageUrl) {
                    $(this).addClass("active");
                    $(this).parent().addClass("menuitem-active");
            ...
            setTimeout(function () {
                var activatedItem = document.querySelector('li.menuitem-active .active');
                if (activatedItem != null) {
                    var simplebarContent = document.querySelector('.leftside-menu-prod .simplebar-content-wrapper');
                    var offset = activatedItem.offsetTop - 300;
                    if (simplebarContent && offset > 100) {
                        scrollTo(simplebarContent, offset, 600);
                    }
